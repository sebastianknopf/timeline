...
| T8 | `stop_times.txt` + timezone context | Group by trip_id in stop_sequence order; extract `stop_sequence` into Timeline facts; parse GTFS time strings in agency timezone (fallback `PROCESSOR_TIMEZONE`), including values >24:00:00, then convert parsed timestamps to `PROCESSOR_TIMEZONE`. Normalize `shape_dist_traveled` to kilometers with meter-detection heuristic and fallback defaults. | Ordered trip stop-time stream in processor timezone |
| T9 | Current-day trip metadata + ordered stop_times | Compute nominal trip aggregates: start/end timestamps, start/end stops, `nom_total_distance`, and `schedule_relationship`. Also set `act_total_distance`, `concessionaire_id`, `concessionaire_name`, `operator_id`, `operator_name` to null at trip level. Build unique `dim_routes` records keyed by mapped `route_id`, capturing `route_name`, `concessionaire_id`, `concessionaire_name` from the route source; `operator_id` and `operator_name` are null for this pipeline. | `dim_trips` nominal rows, `dim_routes` rows |
...
| `fact_stop_times` | `distance_from_start` | `stop_times.shape_dist_traveled` or fallback | Normalize to kilometers. If source seems meter-based, divide by 1000. Missing/unplausible values become `0.0`. Because `fact_stop_times` is keyed by `stop_sequence` (not `distance_from_start`), corrected or updated distance values from later pipeline runs are applied via upsert to existing rows, preventing stale distance accumulation. |
